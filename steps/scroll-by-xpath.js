...
module.exports = async ({ page, options }) => {
  if (!validator.equals(options?.type, STEP_TYPE["SCROLL_BY_XPATH"]))
    throw new Error("SCROLL_XPATH.STEP_TYPE.INVALID");
  ...
  if (!validator.isInt(options?.elementIndex))
    throw new Error("SCROLL_XPATH.ELEMENT_INDEX.INVALID");
